    ...
    <div class="container">
        <div class="mt-20 flex flex-wrap w-screen justify-center">
            @if ($wishlists->isEmpty())
                <div class="flex text-center font-bold text-xl">There are no wishlist yet!</div>
            @else
                    ...
                    <div
                        ...
                        <div class="flex absolute w-12 h-12 bg-white/50 rounded-full top-0 right-0 mt-2 mr-2">
                            <form class="flex mx-auto self-center wishlistForm"
                                action="/removeFromWishlist/{{ $w->id }}" method="POST">
                                @csrf
                        ...
                        <a href="/detail/{{ $w->pet->id }}">
                            <img class="object-cover rounded-t-xl h-48 w-48"
                                src="{{ URL::asset('storage/' . $w->pet->image) }}">
                        </a>
                        <div class="text-center my-auto">{{ $w->pet->name }}</div>
                    </div>
                @endforeach
            @endif
        </div>
